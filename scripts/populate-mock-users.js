...
const mockUsers = [
  {
    ...
    preferences: {
      primary_reason: 'family_pressure',
      relationship_type: 'platonic',
      wants_children: true,
      children_timeline: '3_5_years',
      housing_preference: 'separate_spaces',
      financial_arrangement: 'separate',
      income_level: '100k_200k',
      willing_to_relocate: false,
      public_relationship: true,
  ...
  },
  {
    ...
    preferences: {
      primary_reason: 'legal_benefits',
      relationship_type: 'platonic',
      wants_children: false,
      housing_preference: 'roommates',
      financial_arrangement: 'prenup_required',
      income_level: '50k_100k',
  ...
  },
  {
    ...
    preferences: {
      ...
      wants_children: true,
      children_timeline: '1_3_years',
      housing_preference: 'shared_bedroom',
      financial_arrangement: 'joint',
      income_level: '50k_100k',
      willing_to_relocate: false,
      public_relationship: true,
...
async function createMockUser(userData) {
  try {
    ...
    for (let i = 0; i < userData.photos.length; i++) {
      ...
      if (photoError) {
        console.error(`Error creating photo for ${userData.email}:`, photoError);
      }
    }
    ...
    if (prefsError) {
      console.error(`Error creating preferences for ${userData.email}:`, prefsError);
    } else {
      console.log(`Created preferences for ${userData.profile.display_name}`);
    }

    console.log(`✅ Successfully created mock user: ${userData.profile.display_name}\n`);
...
async function populateMockUsers() {
  console.log('🚀 Starting to populate mock users...\n');

  for (const userData of mockUsers) {
    await createMockUser(userData);
  }

  console.log('\n✨ Finished populating mock users!');
  console.log('You can now see these profiles in the Discover screen of your app.');
}

// Run the script
populateMockUsers().catch(console.error);